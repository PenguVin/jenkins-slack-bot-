...
def get_user_display_name(user_id):
    ...
    return profile.get('display_name') or profile.get('real_name') or user_info['user']['name']
...
def create_input_element(param):
    ...
    is_date = (any(indicator in param_name.lower() for indicator in date_indicators) or
               any(indicator in param.get('description', '').lower() for indicator in date_indicators) or
               re.match(r'^\d{4}-\d{2}-\d{2}$', default_value))
    ...
    if is_date:
        ...
        if 'start' in param_name_lower:
            ...
            default_date = prev_month.replace(day=1).strftime('%Y-%m-%d')
        elif 'end' in param_name_lower:
            prev_month = today.replace(day=1) - timedelta(days=1)
            last_day = calendar.monthrange(prev_month.year, prev_month.month)[1]
            default_date = prev_month.replace(day=last_day).strftime('%Y-%m-%d')
        else:
...
def run_jenkins_job(job_name, params, respond_func, user_id=None):
    ...
    try:
        url = f"{os.getenv('JENKINS_URL')}/job/{job_name}/api/json"
        ...
        if not trigger_job_with_params(job_name, params):
